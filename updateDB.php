<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('logs', "create table if not exists `logs` (   `Id` INT unsigned not null auto_increment , primary key (`Id`), `ip` VARCHAR(16) , `ts` DATETIME , `Details` TEXT ) CHARSET latin1", $silent, array( " ALTER TABLE `logs` CHANGE `ts` `ts` DATETIME "));
